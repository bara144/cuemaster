...
import { Save, AlertCircle, Languages, Type, RefreshCcw, RotateCw, ShieldAlert, CheckCircle2, Trash2, Zap, MinusCircle, HelpCircle, Star, ShieldCheck, UserPlus, X, Database, Download, RotateCcw as ResetIcon, AlertTriangle, Mail, ShoppingBasket, Plus, DollarSign } from 'lucide-react';
...
const AdminSettings: React.FC<AdminSettingsProps> = ({ settings, setSettings, backups, onRestore, setBackups, t, currentUser }) => {
  ...
  const downloadFullBackup = () => {
    const data = {
      sessions: JSON.parse(localStorage.getItem('cuemaster_sessions') || '[]'),
      transactions: JSON.parse(localStorage.getItem('cuemaster_transactions') || '[]'),
      users: JSON.parse(localStorage.getItem('cuemaster_users') || '[]'),
      settings: JSON.parse(localStorage.getItem('cuemaster_settings') || '{}'),
      players: JSON.parse(localStorage.getItem('cuemaster_players') || '[]'),
      attendance: JSON.parse(localStorage.getItem('cuemaster_attendance') || '[]'),
      tournamentData: JSON.parse(localStorage.getItem('cuemaster_tournament_data') || 'null'),
      tournamentHistory: JSON.parse(localStorage.getItem('cuemaster_tournament_history') || '[]'),
    };
    ...
    link.href = url;
    link.download = `BigBoss_FullBackup_${new Date().toISOString().split('T')[0]}.json`;
  ...
  const deleteBackup = (date: string) => {
    setBackups(prev => prev.filter(b => b.date !== date));
  };
